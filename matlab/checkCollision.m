...
function col = checkCollision(robot, qList, ppoint, pnormal)
    ...
    for i = 1:size(qList, 1)
        ...
        for k = 1:robot.model.n
            ...
            [col, pcol] = linePlaneIntersection(p1, p2, ppoint, pnormal);

            % if there is collision, stop
            if col
                % plot this collision so we can see where
                plotCollision(robot, qList(i, :), pnormal, ppoint, pcol);
                return;
            end
        end
    end
...
function [ret, pcol] = linePlaneIntersection(p1, p2, pp, pn)
...
function plotCollision(robot, q, n, p, pcol)

    figure()

    % plot plane
    w = null(n);                    % Find two orthonormal vectors which are orthogonal to v
    mx = linspace(-0.2, 1.2, 6);
    my = linspace(0, 1.1, 6);
    [P,Q] = meshgrid(mx, my);           % Provide a gridwork (you choose the size)
